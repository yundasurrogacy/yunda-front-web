...
const toZhPath = (loc: string) => (loc === '/' ? '/zh' : `/zh${loc}`)
const buildAlternatives = (loc: string) => ([
...
export default defineNuxtConfig(async () => {
  ...
  const blogRoutes = blogEntries.map(blog => blog.loc)
  ...
  const chineseRoutes = [
    ...
    ...blogRoutes.map(route => toZhPath(route)),
  ]
  ...
  return {
    ...
    sitemap: {
      ...
      urls: async () => {
        ...
        const blogUrls = blogEntries.flatMap(blog => ([
          {
            ...blog,
