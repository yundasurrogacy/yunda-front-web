...
const installmentSteps = computed(() => [
  {
    title: tt('Installment 1: After signing → $56,850 into escrow', '第一笔：签署协议后 → 汇入 $56,850 到信托账户'),
    items: [
      tt('Agency service fee (incl. international coordination): $50,000', '中介服务费（含国际协调）：$50,000'),
      tt('Surrogate recruitment, screening, matching, and full-cycle management', '代母招募、筛选、匹配与全程管理'),
      tt('Dedicated project manager, scheduling, and coordination', '专案客户经理、行程安排与流程协调'),
      tt('Psychological support coordination; re-matching if medically not cleared', '心理支持协调；如因医学筛查不合格导致代母重配等'),
      tt('Escrow management fee: $1,850', '信托账户管理费：$1,850'),
      tt('Administrative fee: $5,000 (screening coordination, records整理, clearance, translation support, birth certificate assistance)', '行政服务费：$5,000（含医疗前期筛查协调、记录整理与“清关”、翻译支持、新生儿出生证明协助等）'),
    ],
  },
  {
    title: tt('Installment 2: After medical clearance → $83,500 into escrow', '第二笔：代母医学清关通过后 → 汇入 $83,500 到信托账户'),
    items: [
      tt('Surrogate compensation: $50,000 (subject to matched surrogate)', '代母补偿金：$50,000（以实际匹配代母为准）'),
      tt('Legal fees: intended parents attorney $3,500; surrogate attorney $1,500', '法律费用：准父母律师 $3,500；代母律师 $1,500'),
      tt('Surrogate medical insurance: $25,000 (reimbursed) + life insurance $1,000', '代母医疗保险：$25,000（实报实销）+ 代母人寿保险 $1,000'),
      tt('Transfer & early pregnancy bonus estimate: $2,500 (pre/post checks and early OB visits)', '胚胎移植及孕早期奖励预估：$2,500（含移植前后检查与初期产检等）'),
    ],
  },
  {
    title: tt('Installment 3: After heartbeat → $17,500 + keep ≥ $10,000 balance', '第三笔：胎心确认后 → 汇入 $17,500 且账户余额保持 ≥ $10,000'),
    items: [
      tt('Clothing allowance $1,000', '服装津贴 $1,000'),
      tt('Monthly living stipend ($300/month) $5,500', '每月生活津贴（$300/月）$5,500'),
      tt('Childcare allowance ($250/month) $4,500', '儿童照护补助（$250/月）$4,500'),
      tt('Parentage order fee $6,500', '亲子权利判决费 $6,500'),
    ],
  },
])
