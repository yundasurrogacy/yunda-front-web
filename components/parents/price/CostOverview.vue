<script setup lang="ts">
</script>

<template>
  <section class="steps-timeline w-full flex justify-center">
    <div class="container max-w-300">
      <!-- Title -->
      <h2 class="title">
        {{ $t('parentsPrice.stepsTimeline.title') }}
      </h2>

      <!-- Desktop Curved Timeline -->
      <div class="desktop-timeline">
        <div class="timeline-grid">
          <!-- Row 1: Start Point -->
          <div class="timeline-row row-1">
            <div class="timeline-left" />
            <div class="timeline-center">
              <div class="start-dot" />
            </div>
            <div class="timeline-right" />
          </div>

          <!-- Row 2: Step 1 (Left) -->
          <div class="timeline-row row-2">
            <div class="timeline-left">
              <div class="step-content">
                <div class="step-inner">
                  <div class="step-circle">
                    <span class="step-number">1</span>
                  </div>
                  <div class="step-text">
                    <h3 class="step-title">
                      {{ $t('parentsPrice.stepsTimeline.step1.title') }}
                    </h3>
                    <p class="step-subtitle">
                      {{ $t('parentsPrice.stepsTimeline.step1.subtitle') }}
                    </p>
                  </div>
                </div>
              </div>
            </div>
            <div class="timeline-center" />
            <div class="timeline-right" />
          </div>

          <!-- Row 3: Step 2 (Right) -->
          <div class="timeline-row row-3">
            <div class="timeline-left" />
            <div class="timeline-center" />
            <div class="timeline-right">
              <div class="step-content">
                <div class="step-inner step-inner-reverse">
                  <div class="step-circle">
                    <span class="step-number">2</span>
                  </div>
                  <div class="step-text step-text-right">
                    <h3 class="step-title">
                      {{ $t('parentsPrice.stepsTimeline.step2.title') }}
                    </h3>
                    <p class="step-subtitle">
                      {{ $t('parentsPrice.stepsTimeline.step2.subtitle') }}
                    </p>
                  </div>
                </div>
              </div>
            </div>
          </div>

          <!-- Row 4: Step 3 (Left) -->
          <div class="timeline-row row-4">
            <div class="timeline-left">
              <div class="step-content">
                <div class="step-inner">
                  <div class="step-circle">
                    <span class="step-number">3</span>
                  </div>
                  <div class="step-text">
                    <h3 class="step-title">
                      {{ $t('parentsPrice.stepsTimeline.step3.title') }}
                    </h3>
                    <p class="step-subtitle">
                      {{ $t('parentsPrice.stepsTimeline.step3.subtitle') }}
                    </p>
                  </div>
                </div>
              </div>
            </div>
            <div class="timeline-center" />
            <div class="timeline-right" />
          </div>

          <!-- Row 5: Step 4 (Right) -->
          <div class="timeline-row row-5">
            <div class="timeline-left">
              <div class="step-duration">
                <p class="duration-text">
                  {{ $t('parentsProcess.stepsTimeline.step4.duration') }}
                </p>
              </div>
            </div>
            <div class="timeline-center" />
            <div class="timeline-right">
              <div class="step-content">
                <div class="step-inner step-inner-reverse">
                  <div class="step-circle">
                    <span class="step-number">4</span>
                  </div>
                  <div class="step-text step-text-right">
                    <h3 class="step-title">
                      {{ $t('parentsProcess.stepsTimeline.step4.title') }}
                    </h3>
                    <p class="step-subtitle">
                      {{ $t('parentsProcess.stepsTimeline.step4.subtitle') }}
                    </p>
                    <p class="step-duration-mobile">
                      {{ $t('parentsProcess.stepsTimeline.step4.duration') }}
                    </p>
                  </div>
                </div>
              </div>
            </div>
          </div>

          <!-- Row 6: Step 5 (Left) -->
          <div class="timeline-row row-6">
            <div class="timeline-left">
              <div class="step-content">
                <div class="step-inner">
                  <div class="step-circle">
                    <span class="step-number">5</span>
                  </div>
                  <div class="step-text">
                    <h3 class="step-title">
                      {{ $t('parentsProcess.stepsTimeline.step5.title') }}
                    </h3>
                    <p class="step-subtitle">
                      {{ $t('parentsProcess.stepsTimeline.step5.subtitle') }}
                    </p>
                    <p class="step-duration-mobile">
                      {{ $t('parentsProcess.stepsTimeline.step5.duration') }}
                    </p>
                  </div>
                </div>
              </div>
            </div>
            <div class="timeline-center" />
            <div class="timeline-right">
              <div class="step-duration">
                <p class="duration-text">
                  {{ $t('parentsProcess.stepsTimeline.step5.duration') }}
                </p>
              </div>
            </div>
          </div>

          <!-- Row 7: Step 6 (Right) -->
          <div class="timeline-row row-7">
            <div class="timeline-left">
              <div class="step-duration">
                <p class="duration-text">
                  {{ $t('parentsProcess.stepsTimeline.step6.duration') }}
                </p>
              </div>
            </div>
            <div class="timeline-center" />
            <div class="timeline-right">
              <div class="step-content">
                <div class="step-inner step-inner-reverse">
                  <div class="step-circle">
                    <span class="step-number">6</span>
                  </div>
                  <div class="step-text step-text-right">
                    <h3 class="step-title">
                      {{ $t('parentsProcess.stepsTimeline.step6.title') }}
                    </h3>
                    <p class="step-subtitle">
                      {{ $t('parentsProcess.stepsTimeline.step6.subtitle') }}
                    </p>
                    <p class="step-duration-mobile">
                      {{ $t('parentsProcess.stepsTimeline.step6.duration') }}
                    </p>
                  </div>
                </div>
              </div>
            </div>
          </div>

          <!-- Row 8: End Point -->
          <div class="timeline-row row-8">
            <div class="timeline-left" />
            <div class="timeline-center">
              <div class="end-dot" />
            </div>
            <div class="timeline-right" />
          </div>
        </div>
      </div>
    </div>
  </section>
</template>

<style scoped>
/* Base Section */
.steps-timeline {
  background-color: var(--yunda-petal);
  padding: 64px 16px;
}

.container {
  margin: 0 auto;
}

/* Title */
.title {
  color: var(--yunda-bark);
  font-size: 40px;
  font-family: var(--font-display);
  font-weight: 600;
  text-align: center;
  margin-bottom: 48px;
}

/* Desktop Timeline */
.desktop-timeline {
  display: block;
}

.timeline-grid {
  display: flex;
  flex-direction: column;
  max-width: 1440px;
  margin: 0 auto;
}

.timeline-row {
  display: flex;
  min-height: 80px;
}

.timeline-left {
  flex: 2;
  position: relative;
  display: flex;
  align-items: center;
  justify-content: flex-start;
}

.timeline-right {
  flex: 2;
  position: relative;
  display: flex;
  align-items: center;
  justify-content: flex-end;
}

.timeline-center {
  position: relative;
  min-width: 20px;
  background-color: transparent;
}

/* Row Specific Borders for Curved Path using Center Column */

/* Row 1: Start point - vertical line down to center */
.row-1 .timeline-center {
  border-bottom-right-radius: 60px;
  border-right: 2px solid var(--yunda-bark);
  border-bottom: 2px solid var(--yunda-bark);
  margin-bottom: -2px;
}
/* Row 2: Step 1 (Left) - connect from center to left */
.row-2 .timeline-left {
  border-left: 2px solid var(--yunda-bark);
  border-top: 2px solid var(--yunda-bark);
  border-bottom: 2px solid var(--yunda-bark);
  border-top-left-radius: 60px;
  border-bottom-left-radius: 60px;
}

.row-2 .timeline-center {
  border-bottom: 2px solid var(--yunda-bark);
}

/* Row 3: Step 2 (Right) - connect from center to right */
.row-3 .timeline-center {
  border-bottom: 2px solid var(--yunda-bark);
}

.row-3 .timeline-right {
  margin-top: -2px;
  border-right: 2px solid var(--yunda-bark);
  border-top: 2px solid var(--yunda-bark);
  border-bottom: 2px solid var(--yunda-bark);
  border-bottom-right-radius: 60px;
  border-top-right-radius: 60px;
}

/* Row 4: Step 3 (Left) - connect from center to left */
.row-4 .timeline-left {
  border-left: 2px solid var(--yunda-bark);
  border-top: 2px solid var(--yunda-bark);
  border-bottom: 2px solid var(--yunda-bark);
  border-top-left-radius: 60px;
  border-bottom-left-radius: 60px;
  margin-top: -2px;
}

.row-4 .timeline-center {
  border-bottom: 2px solid var(--yunda-bark);
}

/* Row 5: Step 4 (Right) - connect from center to right */
.row-5 .timeline-center {
  border-bottom: 2px solid var(--yunda-bark);
}

.row-5 .timeline-right {
  margin-top: -2px;
  border-right: 2px solid var(--yunda-bark);
  border-top: 2px solid var(--yunda-bark);
  border-bottom: 2px solid var(--yunda-bark);
  border-bottom-right-radius: 60px;
  border-top-right-radius: 60px;
}

/* Row 6: Step 5 (Left) - connect from center to left */
.row-6 .timeline-left {
  border-left: 2px solid var(--yunda-bark);
  border-top: 2px solid var(--yunda-bark);
  border-bottom: 2px solid var(--yunda-bark);
  border-top-left-radius: 60px;
  border-bottom-left-radius: 60px;
  margin-top: -2px;
}

.row-6 .timeline-center {
  border-bottom: 2px solid var(--yunda-bark);
}

/* Row 7: Step 6 (Right) - connect from center to right */

.row-7 .timeline-right {
  margin-top: -2px;
  border-right: 2px solid var(--yunda-bark);
  border-top: 2px solid var(--yunda-bark);
  border-bottom: 2px solid var(--yunda-bark);
  border-bottom-right-radius: 60px;
  border-top-right-radius: 60px;
}

/* Row 8: End point - connect from center */
.row-8 .timeline-center {
  border-top: 2px solid var(--yunda-bark);
  border-left: 2px solid var(--yunda-bark);
  border-top-left-radius: 60px;
  margin-top: -2px;
}

/* Start and End Dots */
.start-dot {
  width: 40px;
  height: 40px;
  background-color: var(--yunda-bark);
  border-radius: 50%;
  top: -50px;
  right: -22px;
  position: absolute;
  z-index: 10;
}
.end-dot {
  width: 40px;
  height: 40px;
  background-color: var(--yunda-bark);
  border-radius: 50%;
  bottom: -50px;
  left: -20px;
  position: absolute;
  z-index: 10;
}

/* Step Content */
.step-content {
  background-color: transparent;
  padding: 24px;
  position: relative;
}

.step-inner {
  display: flex;
  align-items: flex-start;
  gap: 16px;
}

.step-inner-reverse {
  flex-direction: row-reverse;
}

.step-circle {
  display: flex;
  align-items: center;
  justify-content: center;
  width: 100px;
  height: 100px;
  background: radial-gradient(
    circle,
    rgba(255, 255, 255, 0.8),
    var(--yunda-bark) 90%,
    rgba(100, 100, 100, 0.1) 100%
  );
  border-radius: 50%;
  box-shadow:
    inset 0 4px 10px rgba(255, 255, 255, 0.6),
    /* 中间顶部高光 */ inset 0 -2px 8px rgba(0, 0, 0, 0.15); /* 中间底部阴影 */
  flex-shrink: 0;
}

.step-number {
  color: var(--yunda-bark);
  font-size: 64px;
  font-family: 'Cormorant', serif;
  font-weight: bold;
  font-style: italic;
  margin-top: -15px;
}

.step-text {
  flex: 1;
  min-width: 0;
}

.step-text-right {
  text-align: left;
}

.step-title {
  color: var(--yunda-bark);
  font-size: 32px;
  font-family: var(--font-text);
  font-weight: 700;
  font-weight: bold;
  margin-bottom: 8px;
}

.step-subtitle {
  color: var(--yunda-bark);
  font-size: 20px;
  margin-bottom: 8px;
}

.step-duration {
  text-align: right;
  align-items: center;
}

.step-inner-reverse .step-duration {
  text-align: left;
  align-items: center;
}

.duration-text {
  color: var(--yunda-bark);
  font-size: 32px;
  font-family: var(--font-display);
  font-weight: 500;
}

.step-duration-mobile {
  color: var(--yunda-bark);
  font-size: 28px;
  font-family: var(--font-display);
  display: none;
}

/* Responsive */
@media (max-width: 767px) {
  .timeline-grid {
    gap: 32px;
  }

  .timeline-row {
    flex-direction: column;
    align-items: flex-start;
    min-height: auto;
  }

  .timeline-left,
  .timeline-right {
    flex: none;
    width: 100%;
    justify-content: flex-start;
    border: none;
    margin: 0;
  }

  .timeline-center {
    display: none;
  }

  .row-2 .step-content,
  .row-3 .step-content,
  .row-4 .step-content,
  .row-5 .step-content,
  .row-6 .step-content,
  .row-7 .step-content {
    margin: 0;
  }

  .start-dot,
  .end-dot {
    position: static;
    margin-bottom: 16px;
  }

  .start-dot {
    margin-top: 16px;
  }

  .step-duration {
    display: none;
  }

  .step-duration-mobile {
    display: block;
  }

  .step-content {
    background-color: transparent;
  }

  /* Smaller fonts for mobile */
  .step-title {
    font-size: 20px;
  }

  .step-subtitle {
    font-size: 14px;
  }

  .step-number {
    font-size: 40px;
  }

  .step-circle {
    width: 60px;
    height: 60px;
  }

  .step-duration-mobile {
    font-size: 20px;
  }
}

@media (min-width: 1024px) {
  .steps-timeline {
    padding: 80px 80px;
  }

  .title {
    font-size: 40px;
    margin-bottom: 64px;
  }
}
</style>
